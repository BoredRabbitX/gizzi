...
const UI = {
    ...
    de: {
        ...
        low: "Begrenzte Menge",
        ok: "Verfügbar",
        alertStock: "Maximal verfügbare Menge erreicht.",
    ...
    hu: {
        ...
        validEmail: "Kérjük adjon meg érvényes e-mail címet",
        validPhone: "Kérjük adjon meg érvényes telefonszámot",
        orderError: "Hiba a rendelés feldolgozásakor",
...
function render() {
    ...
    const activeProducts = products.filter(p => p.Disponibile === 'SI' && !p.Disponibile?.includes('NO'));
    ...
    if (filteredProducts.length === 0) {
        productsContainer.innerHTML = `
            <div style="text-align: center; padding: 60px 20px; grid-column: 1 / -1;">
                <div style="font-size: 3rem; color: var(--color-secondary); margin-bottom: 20px; font-weight: 300;">&mdash;</div>
    ...
    if (currentView === 'all') {
        html = `<div class="products-grid">${shuffleArray(filteredProducts).map(p => createCard(p, nameKey)).join('')}</div>`;
    } else {
